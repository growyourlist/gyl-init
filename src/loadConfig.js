...
const loadConfig = async () => {
	if (
		...
		if (
			...
			throw new Error('One of the environment variables are set but not all ' +
		...
		console.log('Please enter the config details for the AWS account in ' +
		...
		do {
			userInput = await new Promise((resolve) => {
				rl.question('AWS region [e.g. eu-west-1]: ', (answer => resolve(answer)))
			})
		...
		const accessKeyResponse = await iam.getAccessKeyLastUsed({
